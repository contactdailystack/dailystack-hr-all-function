...
import {
  getPayrollRecord,
  getClockRecordsInRange,
  getApprovedLeavesInMonth,
...
function computeTax(taxableIncome: number): number {
  ...
  if (taxableIncome <= 300000) return (taxableIncome - 150000) * 0.05;
  if (taxableIncome <= 500000) return 7500 + (taxableIncome - 300000) * 0.1;
  if (taxableIncome <= 750000) return 27500 + (taxableIncome - 500000) * 0.15;
  if (taxableIncome <= 1000000) return 65000 + (taxableIncome - 750000) * 0.2;
  if (taxableIncome <= 2000000) return 115000 + (taxableIncome - 1000000) * 0.25;
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (payrollRecord) {
      ...
      const response: ApiResponse<PayslipData> = { success: true, data };
      res.json(response);
      return;
    }
    ...
    const unpaidLeaveDays = leaves.reduce(
      (sum, l) => sum + (parseInt(String(l.duration_days)) || 0),
      0
    );
